...
//Rare exceptions to indicate a bug in the _usetabgroup code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_usetabgroup = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_usetabgroup]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.usetabgroup.module"
